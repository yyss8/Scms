<template>
    <ul id='header_phone_navList'>
        <li v-for='(link, linkIndex) in headerLink' :key='`link-${linkIndex}`'>
            ...
            <ul class='sub_Nav_List' v-if='link.hasSub' :id='subNavId'>
                <button class='navbtn' v-for='(sub, subIndex) in link.subLinks' :key='`sub-link-${subIndex}`' @click='onSubClick(sub.url)'><b>{{sub.name}}</b></button>
            </ul>
        </li>
